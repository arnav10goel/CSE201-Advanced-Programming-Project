...
public class Frost_P2 implements Screen {
    SpriteBatch batch;
    Texture img;
    Sprite img_sprite;
    Texture choose;
    ...
    public Frost_P2(MyGdxGame game){
        this.game = game;
        batch = new SpriteBatch();
        float w = Gdx.graphics.getWidth();
        float h = Gdx.graphics.getHeight();
        img = new Texture(Gdx.files.internal("Frost_P2.png"));
    ...
    public void handleTouch() {
        if(Gdx.input.justTouched()){
            ...
            if(touch_x >= right_arrow_sprite.getX() && touch_x <= (right_arrow_sprite.getX() + right_arrow_sprite.getWidth()) && touch_y >= right_arrow_sprite.getY() && touch_y <= (right_arrow_sprite.getY() + right_arrow_sprite.getHeight())){
                game.setScreen(new Spectre_P2(game));
            }
